...
def random_string(length):
    ...
    for i in range(length):
        rand_string.append(random.choice(letters))
...
def send_message(chat_id, text):
    ...
    req = requests.post(url=f'{config["URL"]}/sendMessage', json=answer)
...
def download_file(source, user_folder):
    image_url = f'{config["URL"]}/getFile?file_id={source["file_id"]}'
    image_json = requests.get(image_url).json()  # Запихиваем в json, т.е. читаем его
    image_inner_path = image_json['result']['file_path']
    image_path = f'{config["FILE_URL"]}/{image_inner_path}'  # это путь к картинке
...
def download_photo(source, user_folder):
    image_url = f'{config["URL"]}/getFile?file_id={source[len(source) - 1]["file_id"]}'  # GET строка, которая определяет путь к файлу (getFile это из api телеги)
    image_json = requests.get(image_url).json()  # Запихиваем в json, т.е. читаем его
    image_inner_path = image_json['result']['file_path']
    image_path = f'{config["FILE_URL"]}/{image_inner_path}'  # это путь к картинке
